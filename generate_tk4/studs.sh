...
file="/home/varvara/Latex/generate_tk4/students.txt"
while read line
do
...
echo "$line" >> "$a".txt
pdflatex -jobname="$a" '\def\teachers{}\input{geom_tk4}'
/usr/lib/sagemath/sage "$a".sagetex.sage
pdflatex -jobname="$a" '\def\teachers{}\input{geom_tk4}'
pdflatex -jobname="$a" -output-directory=/home/varvara/Latex/generate_tk4/stud '\input{geom_tk4}'
done < $file
find . -type f ! -name "*.pdf" ! -name "*.sty" ! -name "geom_tk4*" ! -name "students.txt" ! -name "*.sh" -delete
find . -name '*sage-plots*' ! -name "*geom_tk4*" -exec rm -rf {} +
