...
file="/home/varvara/Latex/generate_tk2/students.txt"
while read line
do
...
echo "$line" >> "$a".txt
pdflatex -jobname="$a" '\def\teachers{}\input{geom_tk2}'
/usr/lib/sagemath/sage "$a".sagetex.sage
pdflatex -jobname="$a" '\def\teachers{}\input{geom_tk2}'
pdflatex -jobname="$a" -output-directory=/home/varvara/Latex/generate_tk2/stud '\input{geom_tk2}'
done < $file
find . -type f ! -name "*.pdf" ! -name "*.sty" ! -name "geom_tk2*" ! -name "students.txt" ! -name "*.sh" -delete
find . -name '*sage-plots*' ! -name "*geom_tk2*" -exec rm -rf {} +
